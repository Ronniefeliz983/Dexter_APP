...
st.markdown("""
<style>
    /* ... (Tu CSS existente va aquí, no lo he modificado) ... */
    ...
    [data-testid="stDataFrame"] thead th { font-weight: 700 !important; }
...
# --- ¡MODIFICADO! 'consultar_usuario' ahora usa HASHING con fallback ---
    ...
    try:
        with engine.connect() as conn:
            ...
            if user_data:
                ...
                if check_password(password_input, hashed_password_from_db):
...
# --- ¡MODIFICADO! 'verificar_login' ahora muestra el CRUD ---
def verificar_login():
...
# --------------------------
# ... (Todas tus funciones de cálculo: calcular_pyme_y_vence, calcular_vencido, get_earliest_batch_initial_cohort, calcular_kpis)
# ... (Estas funciones no cambian, las incluyo por completitud)
...
def render_admin_crud_page():
    ...
    * **Crear/Actualizar:** Siempre debe ingresar una contraseña. Esta se guardará encriptada (hasheada).
    * **Migración:** Si un usuario tiene una contraseña antigua (texto plano), podrá iniciar sesión. Al **actualizar** ese usuario aquí (incluso si no cambia la contraseña, solo dándole a "Actualizar"), su contraseña se encriptará.
    """)
    ...
    with tab3:
        ...
        if user_to_edit:
            ...
            with st.form("update_user_form"):
                ...
                # Campos pre-llenados
                update_nombre = st.text_input("Nombre Completo", value=user_data.get('nombre_supervisor', ''), key="update_name")
                ...
                update_sup_id = st.text_input("Supervisor ID", value=user_data.get('supervisor_id', ''), key="update_sup_id")
                ...
                if submitted_delete:
                    ...
                    # Usamos un botón de confirmación fuera del form (o un check)
                    # Para simplicidad, lo haremos directo, pero en producción se usa un modal
                    if st.button("Sí, estoy seguro, ELIMINAR"):
