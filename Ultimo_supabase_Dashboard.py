...
# --------------------------
st.set_page_config(page_title="Dashboard Trabajos S - v2.7.6", layout="wide")
...
# --------------------------
# Sidebar (Definición Temprana para Filtro)
# --------------------------
# Esto debe ocurrir ANTES de la lógica principal para que 'supervisor_sel' exista
...
# Placeholder para las opciones que se llenarán después
if st.session_state.user_role in ["admin", "gerencia", "supervisor_old"]:
    supervisor_sel = st.sidebar.selectbox("Supervisor", ["Todos"])
else:
    supervisor_sel = st.session_state.supervisor_id
estatus_sel = st.sidebar.multiselect("Estado", options=["Cargando..."], default=["Cargando..."])


# ==============================================================================
# --- LÓGICA PRINCIPAL (v2.7.6 - LÓGICA "PENDIENTES + CERRADOS HOY") ---
# ==============================================================================
...
if df_full_historial is None or df_full_historial.empty:
    ...
    try:
        ...
        # 3. Filtrar por Supervisor (para Tracking, Búsqueda, y base del dashboard)
        ...
        # 4. Crear el 'df_unicos_base' para el DASHBOARD
        # (Todos los pendientes + los cerrados/finalizados de HOY)
        if not df_supervisor_unicos_MASTER.empty:
        ...
        st.error(f"Error fatal al procesar la lógica principal: {e}")
        st.error(traceback.format_exc())
        df_unicos_base = pd.DataFrame(columns=df_full_historial.columns)
        df_supervisor_unicos_MASTER = pd.DataFrame(columns=df_full_historial.columns)
...
def obtener_datos_unicos(df_input):
    ...
    if 'OrdenExterna' not in df_input.columns:
        # st.error("Columna 'OrdenExterna' no encontrada.") # Oculto para no molestar
...
def filtrar_dataframe_con_historial(df_completo_historial, df_unicos_para_buscar, texto_busqueda, supervisor_filter=None):
    ...
    if not texto_busqueda:
        # Si no hay texto, devolvemos un DF vacío (no la lista de "unicos")
    ...
    try:
        # Buscar en las columnas de texto clave
    ...
    # Si se pasó un filtro de supervisor (ej. en modo supervisor), se aplica
...
def display_kpi_metrics(kpis, page_key, critical_metric_key=None, critical_delta_text="Críticos"):
    ...
    if page_key == "principal":
        ...
        if st.session_state.user_role == "admin":
            ...
            metric_with_critical(col3, "🚀 Total Iniciado (Hoy)", 'Total_Iniciado')
...
def display_detail_table(df_data_filtrada, df_full_historial, df_unicos_para_buscar, role, role_supervisor_id, global_supervisor_sel, page_key, file_name_prefix):
    ...
    # df_display_original es la vista filtrada (Pendientes + Cerrados Hoy)
    df_display_original = formatear_para_display(df_data_filtrada.copy() if df_data_filtrada is not None else pd.DataFrame())

    if texto_busqueda:
        # --- Lógica de Búsqueda (v2.7.6) ---
        # Busca en TODOS los tickets del supervisor (df_unicos_para_buscar)
        supervisor_filter = None
        if role == "supervisor":
            supervisor_filter = role_supervisor_id
        # No es necesario filtrar por supervisor para admin/gerencia porque df_unicos_para_buscar ya está filtrado
        
        df_display_filtrado = filtrar_dataframe_con_historial(
            df_full_historial,        # El historial completo (Haystack)
            df_unicos_para_buscar,    # Los únicos MASTER del supervisor (para encontrar IDs)
            texto_busqueda, 
            supervisor_filter
        )
        df_display_final = formatear_para_display(df_display_filtrado)
    else:
        # Si no hay búsqueda, muestra la vista filtrada (Pendientes + Cerrados Hoy)
...
def render_hourly_efficiency_chart(df_page_data, df_full_historial, chart_key="hourly_efficiency_chart"):
    ...
    try:
        ...
        # Filtrar solo los de hoy para el gráfico de horas
        fecha_hoy = get_current_ast_time().date()
...
# --- FUNCIÓN MODIFICADA (v2.7.6) ---
def render_dashboard_page(title_prefix, df_page_data, df_full_historial, df_unicos_para_buscar, role, role_supervisor_id, global_supervisor_sel, status_filter, page_key, critical_metric_key=None):
    """
    ...
    if role == "admin":
        ...
        if page_key == "principal":
            ...
            col_kpi3.metric("Total Iniciado (Hoy)", total_iniciado_admin)
...
# --------------------------
# Sidebar (Llenado final)
# --------------------------
# df_unicos_base ya está filtrado por la lógica (Pendientes + Cerrados Hoy)
# df_supervisor_unicos_MASTER tiene todos los tickets del supervisor

# Llenar las opciones de filtro basado en los datos de HOY
...
if df_unicos_base is not None and not df_unicos_base.empty:
    if 'Supervisor' in df_unicos_base.columns:
        # Usamos df_supervisor_unicos_MASTER para las opciones de supervisor, por si acaso
        supervisores_validos = sorted([str(s) for s in df_supervisor_unicos_MASTER['Supervisor'].dropna().unique() if str(s).strip()])
...
# Re-crear los widgets del sidebar CON las opciones
if st.session_state.user_role in ["admin", "gerencia", "supervisor_old"]:
    supervisor_sel = st.sidebar.selectbox("Supervisor", supervisor_options, key="supervisor_filter")
else:
    supervisor_sel = st.session_state.supervisor_id
estatus_sel = st.sidebar.multiselect("Estado", options=estado_options, default=estado_options, key="status_filter")


# --- DataFrames Filtrados (Basados en 'df_unicos_base') ---
# df_unicos ya está filtrado por supervisor (gracias a df_unicos_base)
# Ahora solo filtramos por estado
...
if menu == "🏠 Principal":
    ...
    st.info("Mostrando tickets Pendientes (todos) + tickets Cerrados (hoy).") # <-- Aclaración
    ...
    render_dashboard_page(
        ...
        df_full_historial=df_full_historial,
        df_unicos_para_buscar=df_supervisor_unicos_MASTER[df_supervisor_unicos_MASTER['Es_PYME_Negocio'] == True],
        role=st.session_state.user_role,
        role_supervisor_id=st.session_state.supervisor_id,
        global_supervisor_sel=supervisor_sel,
        status_filter=estatus_sel,
    ...
    render_dashboard_page(
        ...
        df_full_historial=df_full_historial,
        df_unicos_para_buscar=df_supervisor_unicos_MASTER, # La búsqueda sigue siendo general
        role=st.session_state.user_role,
        role_supervisor_id=st.session_state.supervisor_id,
        global_supervisor_sel=supervisor_sel,
        status_filter=estatus_sel,
    ...
    render_dashboard_page(
        ...
        df_full_historial=df_full_historial,
        df_unicos_para_buscar=df_supervisor_unicos_MASTER, # Búsqueda general
        role=st.session_state.user_role,
        role_supervisor_id=st.session_state.supervisor_id,
        global_supervisor_sel=supervisor_sel,
        status_filter=estatus_sel,
...
# --- PÁGINA "TRACKING TICKET" (v2.7.6) ---
    ...
    if ticket_busqueda:
        # Llamar a la función de búsqueda usando el historial COMPLETO
        df_track = filtrar_dataframe_con_historial(
            df_full_historial,            # El historial completo (Haystack)
            df_supervisor_unicos_MASTER,  # Los únicos de todo el historial (para encontrar IDs)
            ticket_busqueda, 
            None # El filtro de supervisor ya está en df_supervisor_unicos_MASTER
        )
        ...
        # Usa el MASTER (todos los tickets del supervisor)
