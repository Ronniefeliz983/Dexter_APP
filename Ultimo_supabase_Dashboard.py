...
# ==============================================================================
# --- SECCIÓN 1: DEFINICIÓN DE TODAS LAS FUNCIONES ---
# ==============================================================================
...
# --- FUNCIÓN DE KPI MODIFICADA v2.8.0 ---
...
def get_initial_cohort_from_snapshot(df_snapshot):
    ...
    if (df_snapshot is None or df_snapshot.empty or
        ...
        'Estado' not in df_snapshot.columns):
        return set() # Devuelve un set vacío

    try:
        df_initial_active = df_snapshot[
            df_snapshot['Estado'].astype(str).str.lower().isin(['activo', 'iniciado'])
        ]
        initial_cohort_ids = set(df_initial_active['OrdenExterna'].unique())
        return initial_cohort_ids
        
    except Exception as e:
        st.error(f"Error en get_initial_cohort_from_snapshot: {e}")
        return set()

# --- FUNCIÓN KPI MODIFICADA v2.8.0 ---
def calcular_kpis(df, df_snapshot, df_full_historial):
    """
    Calcula los KPIs.
    ...
    # --- KPIs Estándar (Basados en la vista principal 'df') ---
    # Esta lógica ahora es CORRECTA gracias al nuevo filtro de 'df'
    total = len(df_kpi)
    ...
    # Eficiencia = Manejados (hoy) / Total (Pendientes + Manejados Hoy)
    eficiencia_total = round(manejados * 100 / total, 1) if total > 0 else 0.0

    # --- KPIs 'Inicial' (Basados en 'df_snapshot') ---
    ...
    if global_initial_cohort_ids:
        try:
            ...
            # 3. Encontrar la INTERSECCIÓN: Tickets del cohort inicial que están en esta página.
            cohort_tickets_in_current_page_ids = global_initial_cohort_ids.intersection(tickets_en_pagina_actual_ids)
            total_iniciado_en_pagina = len(cohort_tickets_in_current_page_ids) # <-- KPI 'Total Iniciado'

            if total_iniciado_en_pagina > 0:
                # 4. Para la eficiencia, necesitamos el *estado actual* de este grupo,
                #    que leemos del 'df_full_historial'
                df_ultimo_estado_historial = obtener_datos_unicos(df_full_historial)
                df_kpi_del_cohort_en_historial = df_ultimo_estado_historial[
                    df_ultimo_estado_historial['OrdenExterna'].isin(cohort_tickets_in_current_page_ids)
                ]
                
                # 5. Contar cuántos de *ese grupo* están ahora manejados
                cerrados_inicial = df_kpi_del_cohort_en_historial[df_kpi_del_cohort_en_historial['Estado'].isin(['cerrado', 'validacion ext'])].shape[0]
                referidos_inicial = df_kpi_del_cohort_en_historial[df_kpi_del_cohort_en_historial['Estado'] == 'pend trab interno'].shape[0]
                citados_inicial = df_kpi_del_cohort_en_historial[df_kpi_del_cohort_en_historial['Estado'].isin(['pendiente de calendarizacion', 'calendarizado'])].shape[0]
                rebote_inicial = df_kpi_del_cohort_en_historial[df_kpi_del_cohort_en_historial['Estado'] == 'validacion int'].shape[0]
                
                manejados_inicial_en_pagina = cerrados_inicial + referidos_inicial + citados_inicial + rebote_inicial
                
                # 6. Calcular Eficiencia_Inicial
...
def cargar_datos_historial():
    ...
    try:
        ...
        st.error(f"❌ Error al cargar datos desde Supabase (historial_cambios): {e}")
        return pd.DataFrame()
...
def cargar_datos_snapshot():
    ...
    try:
        ...
        df['Estado'] = df['Estado'].astype(str).str.strip().str.lower().replace('nan', None)
        return df
...
def filtrar_dataframe_con_historial(df_completo_historial, df_unicos_para_buscar, texto_busqueda, supervisor_filter=None):
    """
    ...
    if df_unicos_para_buscar is None or df_unicos_para_buscar.empty:
        st.warning("No hay base de datos de tickets únicos para buscar.")
        return pd.DataFrame(columns=df_completo_historial.columns)
...
def display_kpi_metrics(kpis, page_key, critical_metric_key=None, critical_delta_text="Críticos"):
    ...
    kpi_title = "📋 Total (Pendientes + Manejados Hoy)"
    ...
    if page_key == "principal":
        ...
        if st.session_state.user_role == "admin":
            ...
            metric_with_critical(col8, "🔄 Total Manejado (Hoy)", 'Manejados')
            eficiencia_valor = kpis.get('Eficiencia_Total_%', 0.0)
        ...
        if st.session_state.user_role == "admin":
            ...
            metric_with_critical(col3, "✅ Cerrados (Hoy)", 'Cerrados')
            ...
            metric_with_critical(col7, "🔄 Total Manejado (Hoy)", 'Manejados')
            eficiencia_valor = kpis.get('Eficiencia_Total_%', 0.0)
            col8.metric("📊 Eficiencia", f"{eficiencia_valor:.1f}%")


def display_detail_table(df_data_filtrada, df_full_historial, df_unicos_para_buscar, role, role_supervisor_id, global_supervisor_sel, page_key, file_name_prefix):
    """
    Muestra la barra de búsqueda, la tabla de detalles y el botón de descarga.
    df_data_filtrada = Los únicos (Pendientes + Manejados Hoy)
    df_full_historial = El historial COMPLETO.
    df_unicos_para_buscar = El pool de búsqueda (la lista master de únicos del supervisor).
    """
    ...
    df_display_original = formatear_para_display(df_data_filtrada.copy() if df_data_filtrada is not None else pd.DataFrame())
...
def render_hourly_efficiency_chart(df_page_data, df_snapshot, df_full_historial, chart_key="hourly_efficiency_chart"):
    ...
    try:
        ...
        # Filtrar solo los que tienen timestamp de hoy para el gráfico de horas
        df_grafico = df_grafico[df_grafico['Timestamp_Procesado'].dt.date == fecha_hoy]

        if df_grafico.empty:
            st.info("No hay tickets procesados hoy para mostrar en el gráfico de eficiencia por hora.")
            return

        df_grafico['Hora'] = df_grafico['Timestamp_Procesado'].dt.hour
        
        def agg_kpis_por_hora(group):
            kpis_group = calcular_kpis(group, df_snapshot, df_full_historial) 
...
def render_dashboard_page(title_prefix, df_page_data, df_snapshot, df_full_historial, df_unicos_para_buscar, role, role_supervisor_id, global_supervisor_sel, status_filter, page_key, critical_metric_key=None):
    """
    ...
    kpis = calcular_kpis(df_page_data, df_snapshot, df_full_historial)
        ...
        if resumen_admin.empty or resumen_admin['Total'].sum() == 0:
            ...
            try:
                ...
                if page_key == "rendimiento":
                    render_hourly_efficiency_chart(df_page_data, df_snapshot, df_full_historial, chart_key=f"{page_key}_hourly_chart")
            except Exception as e:
    ...
    # --- Vista Gerencia ---
        ...
        display_detail_table(df_page_data, df_full_historial, df_unicos_para_buscar, role, role_supervisor_id, global_supervisor_sel, page_key, page_key)
...
# --- INICIO CAMBIO v2.8.1: Definir todos los estados manejados ---
ESTADOS_MANEJADOS = [
    'cerrado', 'validacion ext', # Finales
    'pend trab interno', 'pendiente de calendarizacion', 'calendarizado', 'validacion int' # Manejados (Pendientes)
]
ESTADOS_PENDIENTES_PUROS = ['activo', 'iniciado']
# --- FIN CAMBIO v2.8.1 ---
...
if df_full_historial is None or df_full_historial.empty:
    ...
    df_unicos_base = pd.DataFrame()
    df_supervisor_unicos_MASTER = pd.DataFrame()
    st.stop()
else:
    # 2. Obtener el estado MÁS RECIENTE de CADA ticket en TODO el historial
    df_ultimo_estado_MASTER = obtener_datos_unicos(df_full_historial)

    # 3. Crear el Sidebar y OBTENER el 'supervisor_sel'
    ...
    if df_ultimo_estado_MASTER is not None and not df_ultimo_estado_MASTER.empty and 'Supervisor' in df_ultimo_estado_MASTER.columns:
        supervisores_validos = sorted([str(s) for s in df_ultimo_estado_MASTER['Supervisor'].dropna().unique() if str(s).strip()])
        supervisor_options.extend(supervisores_validos)

    if st.session_state.user_role in ["admin", "gerencia", "supervisor_old"]:
        supervisor_sel = st.sidebar.selectbox("Supervisor", supervisor_options, key="supervisor_filter_rerun")
    else:
        supervisor_sel = st.session_state.supervisor_id

    # 4. Filtrar por Supervisor (para Tracking, Búsqueda, y base del dashboard)
    df_supervisor_unicos_MASTER = df_ultimo_estado_MASTER.copy()
    if not df_supervisor_unicos_MASTER.empty:
        if st.session_state.user_role == "supervisor":
            if 'Supervisor' in df_supervisor_unicos_MASTER.columns:
                df_supervisor_unicos_MASTER = df_supervisor_unicos_MASTER[df_supervisor_unicos_MASTER['Supervisor'].astype(str) == str(st.session_state.supervisor_id)]
        elif st.session_state.user_role in ["admin", "gerencia", "supervisor_old"] and supervisor_sel != "Todos":
            if 'Supervisor' in df_supervisor_unicos_MASTER.columns:
                df_supervisor_unicos_MASTER = df_supervisor_unicos_MASTER[df_supervisor_unicos_MASTER['Supervisor'].astype(str) == str(supervisor_sel)]

    # --- INICIO DE LA LÓGICA (v2.8.1) ---
    # 5. Crear el 'df_unicos_base' para el DASHBOARD
    if not df_supervisor_unicos_MASTER.empty:
        fecha_hoy = get_current_ast_time().date()
        
        # Regla A: Tickets que siguen 'Activo' o 'Iniciado' (de cualquier fecha)
        mask_pendientes_puros = df_supervisor_unicos_MASTER['Estado'].isin(ESTADOS_PENDIENTES_PUROS)
        
        # Regla B: Tickets que fueron 'Manejados' (Cerrado, Referido, Citado, etc.) HOY
        mask_manejados_hoy = (
            df_supervisor_unicos_MASTER['Estado'].isin(ESTADOS_MANEJADOS) &
            (df_supervisor_unicos_MASTER['Timestamp_Procesado'].dt.date == fecha_hoy)
        )
        
        df_unicos_base = df_supervisor_unicos_MASTER[mask_pendientes_puros | mask_manejados_hoy].copy()
        
        if df_unicos_base.empty:
            st.info("ℹ️ No hay tickets pendientes (de ayer) ni tickets manejados (hoy) para mostrar.")
    ...
    if not df_snapshot_filtrado.empty:
        if st.session_state.user_role == "supervisor":
            if 'Supervisor' in df_snapshot_filtrado.columns:
                df_snapshot_filtrado = df_snapshot_filtrado[df_snapshot_filtrado['Supervisor'].astype(str) == str(st.session_state.supervisor_id)]
        elif st.session_state.user_role in ["admin", "gerencia", "supervisor_old"] and supervisor_sel != "Todos":
            if 'Supervisor' in df_snapshot_filtrado.columns:
                df_snapshot_filtrado = df_snapshot_filtrado[df_snapshot_filtrado['Supervisor'].astype(str) == str(supervisor_sel)]


    # 6. Llenar opciones de estado y crear el filtro de estado
    estado_options = []
    if df_unicos_base is not None and not df_unicos_base.empty and 'Estado' in df_unicos_base.columns:
        estados_validos = sorted([str(e) for e in df_unicos_base['Estado'].dropna().unique() if str(e).strip()])
        estado_options = estados_validos
    
    estatus_sel = st.sidebar.multiselect("Estado", options=estado_options, default=estado_options, key="status_filter_rerun")


# ==============================================================================
# --- SECCIÓN 3: RENDERIZADO DE PÁGINAS ---
# ==============================================================================

# df_unicos: Filtrado final por estado
df_unicos = df_unicos_base.copy() if df_unicos_base is not None else pd.DataFrame()
if not df_unicos.empty and estatus_sel:
    if 'Estado' in df_unicos.columns:
        ...
        df_unicos = pd.DataFrame(columns=df_unicos_base.columns if df_unicos_base is not None else [])
...
if menu == "🏠 Principal":
    ...
    st.info("Mostrando tickets Pendientes (activos/iniciados de cualquier fecha) + tickets Manejados (hoy).")
    ...
    st.info("Mostrando tickets (Pendientes + Manejados Hoy) que son PYME.")
    ...
    df_pymes_para_buscar = df_supervisor_unicos_MASTER.copy()
    if 'Es_PYME_Negocio' in df_pymes_para_buscar.columns:
        df_pymes_para_buscar = df_pymes_para_buscar[df_pymes_para_buscar['Es_PYME_Negocio'] == True]
    ...
    render_dashboard_page(
        ...
        df_full_historial=df_full_historial,
        df_unicos_para_buscar=df_pymes_para_buscar,
        role=st.session_state.user_role,
        role_supervisor_id=st.session_state.supervisor_id,
        global_supervisor_sel=supervisor_sel,
        status_filter=estatus_sel,
    ...
    st.info("Mostrando tickets (Pendientes + Manejados Hoy) con vencimiento 'Hoy'.")
    ...
    st.info("Mostrando tickets (Pendientes + Manejados Hoy) que son Citas Puntuales.")
    hoy = pd.Timestamp.now().normalize()
    ...
    st.info("Mostrando tickets (Pendientes + Manejados Hoy) que cumplen criterios de antigüedad (basado en 'OE_Creacion').")
    hoy = pd.Timestamp.now().normalize()
    ...
    st.info("Esta página filtra los tickets (Pendientes + Manejados Hoy) por la fecha y hora en que fueron PROCESADOS.")
